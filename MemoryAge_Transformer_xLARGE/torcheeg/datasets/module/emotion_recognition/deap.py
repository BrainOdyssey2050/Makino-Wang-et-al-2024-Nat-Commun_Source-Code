...
class DEAPDataset(BaseDataset):
    ...
    def _load_data(file: Any = None,
        ...
        for trial_id in range(len(samples)):
            ...
            # -- before trail processing
            if before_trial:
                # The hook performed on the trial to which the sample belongs. 
                # It is performed before the offline transformation and thus typically used to implement context-dependent sample transformations, 
                # such as moving averages, etc. The input of this hook function is a 2D EEG signal with shape (number of electrodes, number of data points),
                # whose ideal output shape is also (number of electrodes, number of data points).
                trial_samples = before_trial(trial_samples)

            # -- first several seconds as baseline
            trial_baseline_sample = trial_samples[:, :baseline_chunk_size * num_baseline]  # channel(32), timestep(3*128)
            # -- average each second baseline EEG to get the averaged baseline EEG
            ...
            # -- get different ratings for each trial
            ...
            # -- set up start and end point, and moving step
            ...
            while end_at <= trial_samples.shape[1]:
                # get the EEG signal of the current chunk
                ...
                if not offline_transform is None:  
                    ...
                    # -- the processed EEG signal of the current chunk and the corresponding baseline EEG signal
                    t_eeg = t['eeg']
    ...
    def __getitem__(self, index: int) -> Tuple:
        ...
        #   However, if :obj:`self.in_memory` is set to :obj:`False` in default
        eeg = self.read_eeg(eeg_index)
        # the corresponding code for read_eeg:
        ...
        # --- online transform is in the __getitem__, instead of the _load_data --- #
        # e.g., online_transform=transforms.ToTensor(): This transformation is applied to the data at the time of fetching data during training.
